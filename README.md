...
Found a bug or want to improve the pipeline?
...
A: ~1 minute per 1000 sequences on GPU, ~5-10 minutes on CPU

**Q: Do I need CUDA for this?**
A: No, but it's strongly recommended for reasonable runtimes

**Q: Can I use my own pre-trained model?**
A: Yes, modify the model name in embeddings.py:
```python
tokenizer = BertTokenizer.from_pretrained("your_model_name")
model = BertModel.from_pretrained("your_model_name")
```

**Q: What's the difference between 16S and 18S?**
A: 16S is used for prokaryotes (bacteria/archaea), 18S for eukaryotes (fungi/protists)

**Q: Can I combine 16S and 18S data?**
A: Not directly (different genes), but you can generate separate embeddings and visualize together

**Q: How do I interpret confidence scores?**
A: Higher = more confidently assigned to cluster. Low scores (< 0.5) = might be novel or misclassified
